...
export default function TrashManager({projects}:{projects:Project[]}){
  ...
  useEffect(()=>{const sync=()=>setItems(readStoredProjects(projects).filter((p)=>p.status==="trash"));sync();window.addEventListener(PROJECTS_EVENT,sync);window.addEventListener("storage",sync);return()=>{window.removeEventListener(PROJECTS_EVENT,sync);window.removeEventListener("storage",sync);};},[projects]);
