...
export default function BackupManager(){
 ...
 return <><div className="admin-heading"><div><h1>백업과 복원</h1><p>현재 브라우저의 사이트 설정과 프로젝트 데이터를 파일로 보관합니다.</p></div><button className="admin-primary-button" type="button" onClick={download}>새 백업 만들기</button></div><section className="editor-panel"><h2>백업 파일 복원</h2><p className="admin-note">복원하면 현재 브라우저에 저장된 내용이 백업 파일의 내용으로 교체됩니다.</p><input ref={input} type="file" accept="application/json,.json" onChange={restore}/><button type="button" onClick={()=>input.current?.click()}>백업 파일 선택</button></section></>;
}
