...
import { IMAGE_GUIDES, guideText, confirmImageRatio, confirmMixedDetailImage } from "@/lib/image-guidelines";
...
export default function ProjectEditor({ defaults, initialProject }: { defaults: Project[]; initialProject?: Project }) {
  ...
  const uploadDetails = async (event: ChangeEvent<HTMLInputElement>) => { const files = Array.from(event.target.files ?? []); if (!files.length) return; try { for (const file of files) { if (!(await confirmMixedDetailImage(file))) { event.target.value = ""; return; } } const start = project.images.length; const additions = await Promise.all(files.map(async (file, i) => makeProjectImage(await fileToDataUrl(file), project.title, start + i + 1))); patch("images", [...project.images, ...additions]); showAdminToast(`상세 이미지 ${additions.length}장이 추가되었습니다.`, "success"); } catch { showAdminToast("상세 이미지 업로드에 실패했습니다.", "error"); } };
  ...
  return <form className="project-editor" onSubmit={save}>
    ...
    <div className="editor-grid">
      <section className="editor-panel"><h2>기본 정보</h2>
        ...
        <label>카드 비율<select value={project.cardLayout ?? "wide"} onChange={(e) => patch("cardLayout", e.target.value as ProjectCardLayout)}><option value="wide">가로</option><option value="portrait">세로</option><option value="square">정사각형</option></select></label>
      </section>
      <section className="editor-panel"><h2>대표 이미지</h2>{preview && <div className="admin-image-preview"><Image src={preview} alt="대표 이미지 미리보기" width={720} height={480} unoptimized={preview.startsWith("data:")} /></div>}
      ...
      <section className="editor-panel editor-panel-wide"><h2>상세 이미지</h2><label>여러 장 업로드 <span className="admin-image-guide">가로 ({IMAGE_GUIDES.detailLandscape.width} × {IMAGE_GUIDES.detailLandscape.height}px) · 세로 ({IMAGE_GUIDES.detailPortrait.width} × {IMAGE_GUIDES.detailPortrait.height}px)</span><input type="file" accept="image/*" multiple onChange={uploadDetails} /></label><div className="admin-image-list">{project.images.map((image, index) => <article key={image.id}><Image src={image.src} alt={image.alt} width={260} height={180} unoptimized={image.src.startsWith("data:")} /><div><button type="button" onClick={() => moveImage(index, -1)}>↑</button><button type="button" onClick={() => moveImage(index, 1)}>↓</button><button type="button" onClick={() => removeImage(image.id)}>삭제</button></div></article>)}</div></section>
