...
export default function AdminProjectList({ projects }: { projects: Project[] }) {
  ...
  const move = (id: string, direction: -1 | 1) => {
    ...
    saveStoredProjects(next); setItems(next);
  };
  ...
  return <>
    <div className="admin-toolbar">
      ...
      <button className="admin-filter-button" type="button" onClick={() => { window.localStorage.removeItem("fengshui-admin-projects-v3"); window.location.reload(); }}>기본 프로젝트 복원</button>
    </div>
    <div className="admin-table">
      ...
      {filteredProjects.map((project) => <div className="admin-row admin-project-row" key={project.id}>
        ...
        <span className="row-actions"><Link href={`/admin/projects/new/?id=${encodeURIComponent(project.id)}`}>수정</Link><button type="button" onClick={() => remove(project.id)}>삭제</button></span>
      </div>)}
    </div>
  </>;
}
