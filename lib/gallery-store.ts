...
export async function fetchGalleryItems(admin=false):Promise<GalleryItem[]>{try{const r=await fetch(admin?"/api/admin/gallery":"/api/gallery",{credentials:admin?"include":"same-origin",cache:"no-store"});const d=await r.json().catch(()=>null) as {ok?:boolean;items?:GalleryItem[]}|null;if(!r.ok||!d?.ok||!Array.isArray(d.items))throw new Error();if(admin&&d.items.length===0){const legacy=readGalleryItems();if(legacy.length){const migrated=await saveGalleryItemsToServer(legacy);cache(migrated);return migrated}}cache(d.items);return normalize(d.items)}catch{return readGalleryItems()}}
export async function saveGalleryItemsToServer(items:GalleryItem[]):Promise<GalleryItem[]>{
  // 갤러리는 최적화된 data:image 값을 서버에 1장씩 전달한다.
  // 서버의 /api/admin/gallery가 PROJECT_MEDIA.put()을 직접 실행하므로
  // D1에는 R2 URL만 저장되고, R2 저장 실패 시 D1 등록도 성공하지 않는다.
  const stored:GalleryItem[]=[];
  for(const item of items){
    const r=await fetch("/api/admin/gallery",{method:"POST",headers:{"content-type":"application/json"},credentials:"include",body:JSON.stringify({item})});
    const d=await r.json().catch(()=>null) as {ok?:boolean;items?:GalleryItem[];error?:string}|null;
    if(!r.ok||!d?.ok||!Array.isArray(d.items)||!d.items[0])throw new Error(d?.error||"갤러리 R2 저장에 실패했습니다.");
