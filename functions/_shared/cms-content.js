...
export async function materializeKnownImages(bucket, key, value) {
  ...
  if (key === "site") {
    ...
    if (Array.isArray(next?.visual?.pages)) {
      for (let p = 0; p < next.visual.pages.length; p += 1) {
        const elements = next.visual.pages[p]?.elements;
        if (!Array.isArray(elements)) continue;
        for (let i = 0; i < elements.length; i += 1) {
          if (elements[i]?.type === "image" && elements[i]?.src) elements[i].src = await materializeImage(bucket, elements[i].src, `page-${key}-${p + 1}`, `element-${i + 1}`);
        }
      }
    }
  }
...
export async function cleanupReplacedContentImages(bucket, key, previous, next) {
  ...
  if (key === "site") {
    ...
    const nextVisual = new Set();
    for (const page of next?.visual?.pages || []) for (const element of page?.elements || []) if (element?.type === "image" && element?.src) nextVisual.add(element.src);
    for (const page of previous?.visual?.pages || []) for (const element of page?.elements || []) if (element?.type === "image" && element?.src && !nextVisual.has(element.src)) pairs.push([element.src, null]);
  }
